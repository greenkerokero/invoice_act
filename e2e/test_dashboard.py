...
def test_navigation_to_employees(page):
    ...
    page.goto("/employees")
    expect(page).to_have_title(re.compile(r"сотрудники", re.IGNORECASE))
...
def test_navigation_to_unlinked_acts(page):
    ...
    expect(page).to_have_title(re.compile(r"свободные", re.IGNORECASE))


def test_navigation_to_linked_acts(page):
    """Тест: навигация на страницу привязанных актов"""
    page.goto("/linked-acts")
    expect(page).to_have_title(re.compile(r"привязанные", re.IGNORECASE))
