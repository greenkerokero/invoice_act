...
Строго соблюдай следующие правила. Тесты — это индикатор состояния системы.
...
### 1. Анализ источника ошибки

Когда тест падает, не спеши менять код приложения. Сначала проанализируй причину:

1. **Ошибка в приложении (`src/`):** Код работает не так, как задумано в ТЗ
   - *Действие:* Исправь код приложения

2. **Ошибка в тесте (`tests/`):** Тест написан неверно (опечатка, неверная подготовка данных, устаревшая логика)
   - *Действие:* Исправь код теста

### 2. Регламент исправления тестов

Имеешь право менять тесты, если они работают неправильно ("Buggy Tests"):
- **Правило обоснования:** Если меняешь логику теста или проверочные значения — обязан **явно объяснить причину**
- **Запрет на "заглушки":** Нельзя удалять сложные проверки или заменять их на `assert True`

### 3. Алгоритм действий

1. **Запуск:** Запусти тесты
2. **Диагностика:** Определи, кто врет: код или тест
3. **Исправление:**
   - Если виноват код → Правь `src/`
   - Если виноват тест → Правь `tests/`
4. **Верификация:** Снова запусти тесты. Задача не выполнена, пока все тесты не станут зелёными

### 4. Работа с Coverage

- Не удаляй рабочий код приложения ради поднятия процента покрытия
